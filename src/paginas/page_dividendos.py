...
def retornar_dados(ticker, data_inicial, data_final):
    logging.log(
        logging.INFO,
        f"Buscando histórico de {ticker} entre {data_inicial} e {data_final}",
    )
    d = Dividendos()
...
def gerar_bloco_sobre(dados_ticker):
    ...
    if dados_ticker["equity_type_name"] == "Ações":
        ...
        st.markdown(
            f"**Tipo:** {dados_ticker['equity_type_name']} {dados_ticker['specification']}"
        )
        ...
        st.markdown(
            f"**CNPJ da empresa:** {dados_ticker['cnpj']}",
        )
...
def main():
    ...
    if st.button("Pesquisar", help="Pesquisar"):
        ...
        try:
            ...
            sobre, graficos, tabela = st.tabs(
                ["Sobre o Ticker", "📈 Gráficos", "🗃 Dados"]
            )
            ...
            with graficos:
                data_inicial_formatada = DataHoraUtils.retorna_data_formato_ddmmyyyy(
                    data_inicial
                )
                data_final_formatada = DataHoraUtils.retorna_data_formato_ddmmyyyy(
                    data_final
                )

                d = Dividendos()
                d.setar_chave_carteira_global(
                    st.secrets["carteira_global"]["x_api_key"]
                )
                st.markdown(f"## {ticker} - Evolução dos Dividendos e Dividend Yield")
                st.plotly_chart(
                    d.retornar_grafico_evolucao(ticker, dados), use_container_width=True
                )

                st.markdown(f"## {ticker} - Tendência dos Dividendos e Dividend Yield")
                st.plotly_chart(
                    d.retornar_grafico_tendencia(ticker, dados),
                ...
                st.markdown(
                    f"Considerando o período entre {data_inicial_formatada} e {data_final_formatada}."
                )
                st.plotly_chart(
                    d.retornar_grafico_mensal(ticker, dados), use_container_width=True
                )
                ...
                st.plotly_chart(
                    d.retornar_grafico_anual(ticker, dados), use_container_width=True
                )

                st.markdown(
                    f"## {ticker} - Número de vezes que pagou dividendos por ano"
                )
                st.markdown(
                    f"Considerando o período entre {data_inicial_formatada} e {data_final_formatada}."
                )
                st.plotly_chart(
                    d.retornar_grafico_quantidade_pagamentos_anual(ticker, dados),
                    use_container_width=True,
                )
            ...
            mensagens.error(
                "Erro ao buscar os dados. Verifique os parâmetros usados.", icon="🚨"
            )
            st.stop()
