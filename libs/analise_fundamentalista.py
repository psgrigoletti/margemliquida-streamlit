...
class AnaliseFundamentalista():
    ...
    def retorna_trimestre(self, df, possui_data_inicio_exercicio=True):
        if (possui_data_inicio_exercicio and df['MES_INICIAL'] == '01' and df['MES_FINAL'] == '03') or ((not possui_data_inicio_exercicio) and df['MES_FINAL'] == '03'):
            return str(df['ANO']) + '-01-01' 
        if (possui_data_inicio_exercicio and df['MES_INICIAL'] == '04' and df['MES_FINAL'] == '06') or ((not possui_data_inicio_exercicio) and df['MES_FINAL'] == '06'):
            return str(df['ANO']) + '-04-01' 
        if (possui_data_inicio_exercicio and df['MES_INICIAL'] == '07' and df['MES_FINAL'] == '09') or ((not possui_data_inicio_exercicio) and df['MES_FINAL'] == '09'):
            return str(df['ANO']) + '-07-01' 
        if (possui_data_inicio_exercicio and df['MES_INICIAL'] == '10' and df['MES_FINAL'] == '12') or ((not possui_data_inicio_exercicio) and df['MES_FINAL'] == '12'):
    ...
    def gera_df_trimestral(self, df, possui_data_inicio_exercicio=True):
        ...
        df_trimestral['TRIMESTRE'] = df_trimestral.apply(self.retorna_trimestre, axis=1, args=(possui_data_inicio_exercicio,))
    ...
    def gera_grafico_anual(self, df_anual, titulo, descricao_eixo_y, unidade_eixo_y, unidade_y_apos=True):
        fig = go.Figure()
    ...
    def gera_grafico_trimestral(self, df_trimestral, titulo, descricao_eixo_y, unidade_eixo_y, unidade_y_apos=True):
        fig = go.Figure()
    ...
    def gerar_grafico_anual(self):
        ...
        for empresa in self.empresas_selecionadas:
            ...
            lucro_liquido = self.dre_completo.loc[empresa,:].loc['Lucro/Prejuízo Consolidado do Período']
            receita_liquida = self.dre_completo.loc[empresa,:].loc['Receita de Venda de Bens e/ou Serviços']
        ...
        margem_liquida_empresas_anual = self.gera_df_anual(margem_liquida_empresas, [{'indice': 1, 'mes': '-01-'}, {'indice': 2, 'mes': '-12-'}])
        return self.gera_grafico_anual(margem_liquida_empresas_anual, "Margem Liquida", "Percentual", "%")