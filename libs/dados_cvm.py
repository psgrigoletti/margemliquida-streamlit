...
class DadosCVM:
    URL_DADOS_CIAS = 'http://dados.cvm.gov.br/dados/CIA_ABERTA/CAD/DADOS/cad_cia_aberta.csv'
    ...
    def buscar_empresas(self):
        ...
        self.df_dados_empresas = pd.read_csv(self.URL_DADOS_CIAS, sep=";", encoding="ISO-8859-1")
        ...
        self.df_dados_empresas_disponiveis = self.df_dados_empresas[(self.df_dados_empresas['SIT'] == situacao_interesse) & (self.df_dados_empresas['TP_MERC'] == tipo_mercado_interesse)]
    ...
    def __atualiza_dict_df_demonstrativo(self, tipo, arquivo_zip, demonstrativo, ano, codigos_cvm, dict_df):
    ...
    def __processa_arquivos_do_ano(self, ano):
        if not len(self.codigos_cvm) > 0:
            ...
            for demonstrativo in self.demonstrativos_interesse:
                self.__atualiza_dict_df_demonstrativo("itr", arquivo_zip_itr, demonstrativo, ano, self.codigos_cvm, dict_df)
                self.__atualiza_dict_df_demonstrativo("dfp", arquivo_zip_dfp, demonstrativo, ano, self.codigos_cvm, dict_df)
    ...
    def buscar_demonstrativos(self, codigos_cvm, empresas_selecionadas, anos_selecionados, periodos_selecionados):
    ...
    def gerar_dre_completo(self): 
        ...
        self.dre_completo = pd.pivot_table(dre_completo, index=['DENOM_CIA','DS_CONTA'], columns=['DT_INI_EXERC', 'DT_FIM_EXERC'],values=['VL_AJUSTADO'])
