...
class Markowitz:
    ...
    def __init__(self, precos_fechamento, taxa_livre_de_risco, numero_de_portfolios_aleatorios):
    ...
    def __portfolio_annualised_performance(self, pesos):
        ...
        std = np.sqrt(np.dot(pesos.T, np.dot(
            self.__matriz_covariancia, pesos))) * np.sqrt(252)
        return std, returns
    ...
    def __random_portfolios(self):
        ...
        for i in range(self.__numero_de_portfolios_aleatorios):
            ...
            portfolio_std_dev, portfolio_return = self.__portfolio_annualised_performance(
                weights)
            results[0, i] = portfolio_std_dev
            results[1, i] = portfolio_return
            results[2, i] = (portfolio_return -
                             self.__taxa_livre_de_risco) / portfolio_std_dev
    ...
    def retornar_grafico_fronteira_eficiente_aleatorio(self):
        ...
        max_sharpe_allocation = pd.DataFrame(weights[max_sharpe_idx], index=self.__precos_fechamento.columns, columns=['allocation'])
        max_sharpe_allocation.allocation = [round(i*100, 2) for i in max_sharpe_allocation.allocation]
        max_sharpe_allocation = max_sharpe_allocation.T

        min_vol_idx = np.argmin(results[0])
        sdp_min, rp_min = results[0, min_vol_idx], results[1, min_vol_idx]
        min_vol_allocation = pd.DataFrame(weights[min_vol_idx], index=self.__precos_fechamento.columns, columns=['allocation'])
    ...
    def __max_sharpe_ratio(self):
        ...
        result = sco.minimize(self.__neg_sharpe_ratio, num_assets*[1./num_assets,], args=args,
    ...
    def display_simulated_ef_with_random(self):
        ...
        sdp, rp = self.portfolio_annualised_performance(max_sharpe['x'], self.__retornos_medios)
        max_sharpe_allocation = pd.DataFrame(max_sharpe.x, index=self.__precos_fechamento.columns, columns=['allocation'])
        max_sharpe_allocation.allocation = [round(i*100, 2)for i in max_sharpe_allocation.allocation]
        max_sharpe_allocation = max_sharpe_allocation.T

        min_vol = self.min_variance()
        sdp_min, rp_min = self.portfolio_annualised_performance(min_vol['x'], self.__retornos_medios)
        min_vol_allocation = pd.DataFrame(min_vol.x, index=self.__precos_fechamento.columns, columns=['allocation'])
        min_vol_allocation.allocation = [round(i*100, 2)for i in min_vol_allocation.allocation]
        min_vol_allocation = min_vol_allocation.T

        self.retornar_texto_ef_with_random(rp, sdp, max_sharpe_allocation, rp_min, sdp_min, min_vol_allocation)
        self.retornar_grafico_ef_with_random(results, sdp, rp, sdp_min, rp_min)


    
    def retornar_texto_ef_with_random(self, rp, sdp, max_sharpe_allocation, rp_min, sdp_min, min_vol_allocation):
        retorno_textual = "-" * 80
    ...
    def retornar_texto_ef_with_selected(self, rp, sdp, max_sharpe_allocation, rp_min, sdp_min, min_vol_allocation, an_rt, an_vol):
        retorno_textual = "-" * 80
        ...
        for i, txt in enumerate(self.__precos_fechamento.columns):
            retorno_textual += "[" + str(txt) + "]: annuaised return: " + str(round(
                an_rt[i], 2)) + ", annualised volatility: " + str(round(an_vol[i], 2)) + "\n"
        retorno_textual += "-" * 80
        return retorno_textual
...
class GraficosMarkowitz():
    ...
    def retornar_grafico_de_precos_normalizados(self):
        normalizado = self.__markowitz.__precos_fechamento / self.__markowitz.__precos_fechamento.iloc[0]
        fig = normalizado.plot(
    ...
    def retornar_grafico_retornos_diarios(self):
        ...
        for c in self.__markowitz.__retornos_diarios.columns.values:
            plt.plot(self.__markowitz.__retornos_diarios.index,
                     self.__markowitz.__retornos_diarios[c], lw=3, alpha=0.8, label=c)
    ...
    def retornar_matriz_correlacao(self):
        ...
        _ = sns.heatmap(self.__markowitz.__correlacao, annot=True, vmin=-1, vmax=1, cmap='viridis', linewidths=0.5, mask=mascara_grafico)
        plt.title('Matriz de correlação entre os ativos')
        return fig

    def retornar_matriz_risco_retorno(self):
        fig = plt.figure(num=None, figsize=(10, 10), dpi=80, facecolor='w', edgecolor='k')
        _ = sns.heatmap(self.__markowitz.__matriz_covariancia, annot=True, vmin=-1, vmax=1, cmap='viridis', linewidths=0.5)
        plt.title('Matriz de correlação')
        return fig

    def retornar_grafico_vol_e_retornos_medios(self):
        volatilidade = pd.DataFrame(self.__markowitz.__retornos_diarios.std(), columns=['Volatilidade'])
        retornos_medios = pd.DataFrame(self.__markowitz.__retornos_medios, columns=['Retorno'])
        ...
        for i in range(matriz_risco_retorno.shape[0]):
            plt.text(x=matriz_risco_retorno.Volatilidade[i],
    ...
    def retornar_grafico_ef_with_selected(self, an_vol, an_rt, sdp, rp, sdp_min, rp_min):
    ...
    def retornar_grafico_ef_with_random(self, results, sdp, rp, sdp_min, rp_min):
        ...
        plt.plot([p['fun'] for p in self.__markowitz.__efficient_frontier(target)], target, linestyle='-.', color='black', label='Fronteira eficiente')
