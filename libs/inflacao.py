...
class Inflacao:
    ...
    def retornar_grafico_acumulado_12m(self):
        ...
        layout = go.Layout(
            annotations=[
                dict(text=f"Fonte dos dados: SIDRA/IBGE<br>Data: {hoje}",
                     showarrow=False, x=0, y=1,
        ...
        fig = fig.add_trace(go.Scatter(x=self.ipca_12m['date'],
                                       y=self.ipca_12m['value']))
        fig.update_layout(
            title={'text': "<b>IPCA acumulado em 12 meses",
                   'y': 0.9, 'x': 0.5,
    ...
    def retornar_grafico_por_grupo(self):
        hoje = DataHoraUtils.retorna_data_atual_formato_ddmmyyyy()
        colors = ['#282f6b', '#b22200', '#eace3f', '#224f20',
                  '#b35c1e', '#419391', '#839c56', '#3b89bc']

        fig = px.bar(self.ipca_gp_wider,
                     x='date',
                     y='contribuicao',
                     color='groups',
        ...
        fig.update_layout(
            title={'text': f"Contribuições de cada setor no IPCA (Gráfico gerado em {hoje})",
                   'y': 0.9, 'x': 0.5,
                   'xanchor': 'center', 'yanchor': 'top'})
