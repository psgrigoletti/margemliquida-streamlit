...
class Resultado:
    ...
    def __str__(self):
        return (
            f" max: {self.maxima}, min {self.minima}, "
...
class DiasConsecutivos:
    ...
    def retornar_tabela(self):
        for d in self.dias_apos:
            self.resultados[
                d
            ].periodos_encontrados = self.quantidade_periodos_encontrados
        ...
        relatorio += (
            f"#### Número de ocorrências: {self.quantidade_periodos_encontrados}\n"
        )
        ...
        tabela.append(
            [f"| [:green[ACERTOS]] **Variação {resultado_esperado} média** |"]
        )
        tabela.append(["| [:green[ACERTOS]] **Melhor resultado** |"])
        tabela.append(["|  |"])
        tabela.append(["| [:red[ERROS]] **Número de erros** |"])
        tabela.append(
            [f"| [:red[ERROS]] **Variação {resultado_nao_esperado} média** |"]
        )
        ...
        tabela.append(
            [
                '| **[Expectativa matemática](# "EM negativa significa que você irá perder dinheiro no longo prazo; EM positiva significa que você irá ganhar dinheiro no longo prazo.")** |'
            ]
        )
        ...
        for k in self.resultados.keys():
            ...
            if self.direcao == self.direcoes[1]:  # ALTA/UP
                resultados_esperados = [
                    x for x in self.resultados[k].variacoes if x < 0
                ]
                resultados_nao_esperados = [
                    x for x in self.resultados[k].variacoes if x > 0
                ]
            ...
            if len(resultados_esperados) > 0:
                media_melhores_resultado = round(
                    (sum(resultados_esperados) / len(resultados_esperados)), 2
                )
            else:
                media_melhores_resultado = 0

            if len(resultados_nao_esperados) > 0:
                media_piores_resultado = round(
                    (sum(resultados_nao_esperados) / len(resultados_nao_esperados)), 2
                )
            else:
                media_piores_resultado = 0
            ...
            taxa = round(
                (
                    len(resultados_esperados)
                    / (len(resultados_esperados) + len(resultados_nao_esperados))
                )
                * 100,
                2,
            )
            ...
            expectativa_matematica_perdas = abs(
                (1 - (taxa / 100)) * media_piores_resultado
            )
            expectativa_matematica = round(
                expectativa_matematica_ganhos - expectativa_matematica_perdas, 2
            )
    ...
    def retornar_relatorio(self):
        ...
        for k in self.resultados.keys():
            ...
            # media = np.mean(self.resultados[k].variacoes)
            media = sum(self.resultados[k].variacoes) / len(
                self.resultados[k].variacoes
            )
            media = round(media, 2)
            if self.direcao == self.direcoes[1]:  # ALTA/UP
                resultado_esperado = "negativa (-)"
            ...
            lista_variacoes_arredondada = [
                round(x, 2) for x in self.resultados[k].variacoes
            ]
    ...
    def retornar_grafico(self):
        ...
        for tupla in self.periodos:
            ...
            menor_y = (
                round(
                    min(
                        self.df.loc[
                            pd.to_datetime(data_inicial_rect) : pd.to_datetime(
                                data_final_rect
                            )
                        ]["Low"]
                    ),
        ...
        # Define o layout do gráfico
        fig.update_layout(
            title=f"{self.ticker} - Períodos de {self.n_days} {self.direcao.lower()}s consecutivas, entre {self.data_inicial_formatada} e {self.data_final_formatada}",
            xaxis_title="Data",
            yaxis_title="Preço (R$)",
            showlegend=True,
            font=dict(size=12),
    ...
    def __busca_dias_sem_cotacao(self):
        ...
        business_days = (
            b3.schedule(start_date=self.start_date, end_date=self.end_date)
    ...
    def __buscar_dias_consecutivos(self):
        for i in range(self.n_days, len(self.df)):
            ...
            if self.direcao == self.direcoes[1]:  # ALTA/UP
                if all(temp1 < temp2):
                    self.df.loc[
                        maior_data_achou, f"{self.direcao}_{self.n_days}_dias"
                    ] = True
            ...
            self.quantidade_periodos_encontrados = len(
                self.df[self.df[f"{self.direcao}_{self.n_days}_dias"] == True]
            )
