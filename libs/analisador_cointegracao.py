...
class AnalisadorCointegracao:
    ...
    def __init__(self, df, lista_pares, confianca_minima = 0.9, distancia_da_media_interesse = 1.8, # NÚMERO DE DESVIOS
                quantidade_comprar = 100, plotar_cointegracao = True):
    ...
    def plotar_dados(self):
        ...
        # PLOTA O RESIDUAL
        axis[1] = self.df_do_par_atual['Residual'].plot(x="Major", y="", label='Resíduo')
        axis[1].axhline(y=self.mean, color='y', linestyle='--', linewidth=1, label='Média')
        axis[1].axhline(y=up,   color='b', linestyle='--', linewidth=1, label=f'+/-{k} STD')
        axis[1].axhline(y=down, color='b', linestyle='--', linewidth=1, label='_nolegend_')
    ...
    def imprimir_relatorio(self):
        ...
        print("Data inicial dos dados: " + self.df.first_valid_index().strftime("%d/%m/%Y")) # + " (era " + self.data_inicial.strftime("%d/%m/%Y") + ")")
        print("Data final dos dados: " + self.df.last_valid_index().strftime("%d/%m/%Y"))  # + " (era " + self.data_final.strftime("%d/%m/%Y") + ")")
        ...
        valor_gasto_compra = str(round(float(self.df[stock_comprar].iloc[-1] * self.quantidade_comprar), 2))
        quantidade_vender = float(self.beta * float(self.quantidade_comprar))
        valor_recebido_venda = str(round(float(self.df[stock_vender].iloc[-1] * quantidade_vender), 2))
        print(f"COMPRAR {self.quantidade_comprar} {stock_comprar} -> R$ - {valor_gasto_compra}")
        print(f"VENDER {quantidade_vender} {stock_vender} -> R$ + {valor_recebido_venda}")
