...
def validar_parametros(ticker, data_inicial, data_final, dias_analise, mensagens):
    ...
    if data_final <= data_inicial:
        mensagens.error(
            'Data inicial deve ser menor que a Data final.', icon="🚨")
        st.stop()

    if not dias_analise or len(dias_analise) == 0:
        mensagens.error(
            'Selecione pelo menos um valor em \'Analisar n dias após\'.', icon="🚨")
        st.stop()
...
def main():
    ...
    data_inicial = col2.date_input(
        "Data inicial:", value=datetime.date(2020, 1, 1))

    data_final = col3.date_input(
        "Data final:", value=datetime.date(2023, 4, 1))
