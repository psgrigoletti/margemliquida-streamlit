...
def buscar_demonstrativos(_af, empresas_selecionadas, anos_selecionados, periodos_selecionados):
    if (len(empresas_selecionadas)>0 and len(anos_selecionados)>0 and len(periodos_selecionados)>0):
        df_filtradas = _af.df_dados_empresas_disponiveis[_af.df_dados_empresas_disponiveis.DENOM_SOCIAL.str.contains('|'.join(empresas_selecionadas), na=False)]
        ...
        _af.buscar_demonstrativos(codigos_cvm, empresas_selecionadas, anos_selecionados, periodos_selecionados)
